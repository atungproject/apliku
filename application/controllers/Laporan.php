...
class Laporan extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('transaksi', 'Transaksi', 'required|in_list[data_masuk,data_keluar]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $query = '';
            if ($table == 'data_masuk') {
                $query = $this->admin->getDataMasuk(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {
                $query = $this->admin->getDataKeluar(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            }
    ...
    private function _cetak($data, $table_, $tanggal)
    {
        ...
        $table = $table_ == 'data_masuk' ? 'Data Masuk' : 'Data Keluar';
        ...
        if ($table_ == 'data_masuk') :
            $pdf->Cell(10, 7, 'No.', 1, 0, 'C');
            $pdf->Cell(25, 7, 'Tanggal', 1, 0, 'C');
            $pdf->Cell(35, 7, 'ID Transaksi', 1, 0, 'C');
            $pdf->Cell(55, 7, 'Nama Data', 1, 0, 'C');
            $pdf->Cell(45, 7, 'Supplier', 1, 0, 'C');
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(35, 7, $d['id_data_masuk'], 1, 0, 'C');
                $pdf->Cell(55, 7, $d['nama_data'], 1, 0, 'L');
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(35, 7, $d['id_data_keluar'], 1, 0, 'C');
                $pdf->Cell(55, 7, $d['nama_data'], 1, 0, 'L');
